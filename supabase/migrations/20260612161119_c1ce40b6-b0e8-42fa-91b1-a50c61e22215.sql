...
BEGIN
  ...
  ELSE
    ...
    BEGIN
      ...
      IF v_needs_update THEN
        IF v_last_active_updated THEN UPDATE telegram_users SET username = p_username, first_name = p_first_name, last_active = NOW() WHERE id = p_user_id;
        ELSE UPDATE telegram_users SET username = p_username, first_name = p_first_name WHERE id = p_user_id; END IF;
      END IF;
    END;
  END IF;
  ...
  IF v_user.penalty_points >= 100 THEN
    ...
    BEGIN
       ...
       IF NOT v_is_prem THEN
         ...
         VALUES (p_user_id, 'auto_penalty_100', 'Akun diblokir karena terlalu banyak laporan negatif dari pengguna lain.', true)
         ON CONFLICT (user_id) DO UPDATE SET is_active = true, blocked_at = COALESCE(blocked_users.blocked_at, NOW()), unblocked_at = NULL, reason = EXCLUDED.reason, blocked_message = EXCLUDED.blocked_message;
       ELSE
  ...
  IF v_blocked_at IS NOT NULL THEN
    IF v_blocked_at + INTERVAL '15 days' > NOW() THEN
      ...
      v_is_blocked := TRUE;
      v_blocked_until := v_blocked_at + INTERVAL '15 days';
    ELSE
      ...
      v_is_blocked := FALSE;
      v_user.penalty_points := 0;
      v_user.unacknowledged_reports_count := 0;
    END IF;
  END IF;
  ...
  FOR v_candidate IN
    SELECT wq.user_id as candidate_id, tu.gender as candidate_gender, tu.target_gender as candidate_target_gender,
      tu.location as candidate_location, tu.target_location as candidate_target_location, tu.premium_until as candidate_premium_until,
      tu.state as candidate_state, tu.partner_id as candidate_partner_id, tu.last_partners as candidate_last_partners
...
CREATE OR REPLACE VIEW public.v_eligible_reengagement_users AS
SELECT tu.*
FROM telegram_users tu
WHERE NOT EXISTS (
  SELECT 1 FROM blocked_users bu
  WHERE bu.user_id = tu.id
    AND bu.is_active = TRUE
    AND bu.blocked_at + INTERVAL '15 days' > NOW()
);

GRANT SELECT ON public.v_eligible_reengagement_users TO authenticated;
GRANT ALL ON public.v_eligible_reengagement_users TO service_role;