...
BEGIN
  ...
  IF v_premium_until IS NOT NULL AND v_premium_until > NOW() THEN
    RETURN json_build_object('success', TRUE, 'allowed', TRUE, 'is_premium', TRUE,
      'target_gender', v_target_gender, 'target_location', v_target_location);
  END IF;

  -- Non-premium = not allowed
  RETURN json_build_object('success', TRUE, 'allowed', FALSE, 'is_premium', FALSE,
    'target_gender', v_target_gender, 'target_location', v_target_location);
END;
$function$;
...
BEGIN
  ...
  -- Filter hanya untuk premium, non-premium selalu default
  ...
  LOOP
    ...
    -- Kandidat: filter hanya untuk premium
