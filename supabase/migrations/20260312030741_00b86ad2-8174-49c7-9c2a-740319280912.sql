...
-- 1. Update comprehensive_search_action: remove filter limit logic, always allow filters
...
BEGIN
  ...
  IF v_blocked_until IS NOT NULL AND v_blocked_until > NOW() THEN
    RETURN json_build_object('success', false, 'action', 'show_temp_banned', 'error', 'user_temp_banned', 'blocked_until', v_blocked_until);
  END IF;
  IF v_blocked_until IS NOT NULL AND v_blocked_until <= NOW() THEN
  ...
  IF p_is_next AND v_user.state = 'chatting' AND v_user.partner_id IS NOT NULL THEN
    ...
    SET state = 'idle', partner_id = NULL, chat_end_count = COALESCE(chat_end_count, 0) + 1
    WHERE id = p_user_id;
  ...
  -- 8. CARI PARTNER - Filter selalu aktif untuk semua user (no limit)
  ...
  LOOP
    ...
    -- Filter kompatibilitas - semua user bisa pakai filter tanpa limit
  ...
  IF v_partner_id IS NOT NULL THEN
    UPDATE telegram_users SET state = 'chatting', partner_id = v_partner_id WHERE id = p_user_id;
    UPDATE telegram_users SET state = 'chatting', partner_id = p_user_id WHERE id = v_partner_id;
    DELETE FROM waiting_queue WHERE user_id = v_partner_id;
    
    SELECT premium_until INTO v_partner_premium_until FROM telegram_users WHERE id = v_partner_id;
    
    RETURN json_build_object(
      'success', true, 'matched', true, 'partner_id', v_partner_id,
      'action', 'notify_both', 'chat_ended', v_chat_ended,
      'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo,
  ...
  ELSE
    ...
    RETURN json_build_object(
      'success', true, 'matched', false, 'partner_id', NULL,
      'action', 'wait_in_queue', 'chat_ended', v_chat_ended,
      'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo,
      'reputation', json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points)
    );
  END IF;
...
-- 2. Update search_or_next_partner: remove filter limit logic
CREATE OR REPLACE FUNCTION public.search_or_next_partner(p_user_id bigint, p_is_next boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_chat_ended BOOLEAN := FALSE;
BEGIN
  SELECT gender, target_gender, location, target_location, premium_until,
    state, partner_id, COALESCE(penalty_points, 0) as penalty_points
  INTO v_user FROM telegram_users WHERE id = p_user_id;
  
  IF v_user IS NULL THEN
    RETURN json_build_object('success', false, 'error', 'user_not_found', 'action', 'none');
  END IF;
  
  v_penalty_points := v_user.penalty_points;
  IF v_penalty_points >= 100 THEN v_reputation_status := 'banned'; v_reputation_message := 'Akun Anda telah diblokir.';
  ELSIF v_penalty_points >= 70 THEN v_reputation_status := 'critical'; v_reputation_message := 'Akun Anda di ambang pemblokiran.';
  ELSIF v_penalty_points >= 40 THEN v_reputation_status := 'warning'; v_reputation_message := 'Kami menerima laporan negatif tentang Anda.';
  ELSE v_reputation_status := 'good'; v_reputation_message := NULL;
  END IF;
  ...
  IF p_is_next AND v_user.state = 'chatting' AND v_user.partner_id IS NOT NULL THEN
    v_old_partner_id := v_user.partner_id;
    UPDATE telegram_users SET state = 'idle', partner_id = NULL WHERE id = p_user_id AND partner_id = v_old_partner_id;
    IF FOUND THEN
      UPDATE telegram_users SET state = 'idle', partner_id = NULL WHERE id = v_old_partner_id AND partner_id = p_user_id;
      DELETE FROM waiting_queue WHERE user_id IN (p_user_id, v_old_partner_id);
      UPDATE telegram_users SET chat_end_count = COALESCE(chat_end_count, 0) + 1 WHERE id IN (p_user_id, v_old_partner_id);
      v_chat_ended := TRUE;
      SELECT public.handle_end_chat_promo_logic(v_old_partner_id) INTO v_old_partner_promo;
    END IF;
  END IF;
  ...
  IF v_user.state = 'waiting' AND NOT p_is_next THEN
    IF EXISTS (SELECT 1 FROM waiting_queue WHERE user_id = p_user_id) THEN
      RETURN json_build_object('success', false, 'error', 'already_in_queue', 'action', 'show_waiting', 'reputation', json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points));
    ELSE
      UPDATE telegram_users SET state = 'idle' WHERE id = p_user_id;
    END IF;
  END IF;
  
  -- Filter selalu aktif untuk semua user (no limit)
  v_my_gender := v_user.gender;
  v_my_target_gender := v_user.target_gender;
  v_my_location := v_user.location;
  v_my_target_location := v_user.target_location;
  ...
  FOR v_candidate IN
    SELECT wq.user_id as candidate_id, tu.gender as candidate_gender,
      tu.target_gender as candidate_target_gender, tu.location as candidate_location,
      tu.target_location as candidate_target_location, tu.premium_until as candidate_premium_until,
      tu.state as candidate_state, tu.partner_id as candidate_partner_id
    FROM waiting_queue wq JOIN telegram_users tu ON tu.id = wq.user_id
    WHERE wq.user_id != p_user_id ORDER BY wq.joined_at ASC
    FOR UPDATE OF wq SKIP LOCKED
  LOOP
    IF v_candidate.candidate_state != 'waiting' OR v_candidate.candidate_partner_id IS NOT NULL THEN
      DELETE FROM waiting_queue WHERE user_id = v_candidate.candidate_id;
  ...
  IF v_partner_id IS NOT NULL THEN
    ...
    UPDATE telegram_users SET state = 'chatting', partner_id = p_user_id WHERE id = v_partner_id;
    DELETE FROM waiting_queue WHERE user_id = v_partner_id;
    RETURN json_build_object('success', true, 'matched', true, 'partner_id', v_partner_id, 'action', 'notify_both', 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points));
  ELSE
    INSERT INTO waiting_queue (user_id, joined_at) VALUES (p_user_id, NOW()) ON CONFLICT (user_id) DO UPDATE SET joined_at = NOW();
    UPDATE telegram_users SET state = 'waiting' WHERE id = p_user_id;
    RETURN json_build_object('success', true, 'matched', false, 'partner_id', NULL, 'action', 'wait_in_queue', 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points));
  END IF;
END;
$function$;

-- 3. Simplify check_and_use_filter: just return user settings, no limit
...
BEGIN
  ...
  INTO v_premium_until, v_target_gender, v_target_location
  FROM telegram_users
  WHERE id = p_user_id;
  ...
  RETURN json_build_object(
    'success', TRUE, 
    'allowed', TRUE, 
    'is_premium', v_premium_until IS NOT NULL AND v_premium_until > NOW(),
...
-- 4. Drop filter_uses columns (no longer needed)
ALTER TABLE telegram_users DROP COLUMN IF EXISTS filter_uses_today;
ALTER TABLE telegram_users DROP COLUMN IF EXISTS filter_uses_date;
