...
BEGIN
  ...
  -- 2. CEK BLOKIR PERMANEN (dengan auto-expire 30 hari)
  ...
  IF v_blocked_at IS NOT NULL THEN
    IF v_blocked_at + INTERVAL '30 days' > NOW() THEN
      -- Masih dalam masa blokir 30 hari
      SELECT blocked_message INTO v_blocked_message FROM blocked_users WHERE user_id = p_user_id AND is_active = TRUE LIMIT 1;
      v_is_blocked := TRUE;
    ELSE
      -- Sudah lewat 30 hari, auto-unblock dan reset penalty
