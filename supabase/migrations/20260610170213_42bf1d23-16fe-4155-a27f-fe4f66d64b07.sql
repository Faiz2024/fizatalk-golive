...
BEGIN
  SELECT 
    ...
    spam_warnings,
    spam_warning_until
  INTO v_is_premium, v_warnings, v_warning_until
  ...
  IF v_warnings >= 4 THEN
    ...
    SET spam_warnings = 4, spam_warning_until = v_new_warning_until, penalty_points = 100 
    WHERE id = p_sender_id;
    
    RETURN json_build_object('success', true, 'action', 'blocked', 'warnings', 4);
  ELSE
    UPDATE telegram_users 
    SET spam_warnings = v_warnings, spam_warning_until = v_new_warning_until,
