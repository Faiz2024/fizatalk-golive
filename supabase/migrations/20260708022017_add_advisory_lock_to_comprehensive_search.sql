-- Migration to add double advisory lock to comprehensive_search_action to prevent matchmaking race conditions
...
BEGIN
  ...
  IF p_is_next AND v_user.state = 'chatting' AND v_user.partner_id IS NOT NULL THEN
    -- MENCEGAH CHAT PUTUS OTOMATIS:
    -- Jika p_target_partner_id IS NULL tapi DB mengatakan state = chatting,
    -- itu artinya perintah /next ini berasal dari masa lalu (saat user masih waiting).
  ...
  LOOP
    ...
    IF v_i_satisfied_gender AND v_candidate_satisfied_gender AND v_i_satisfied_location AND v_candidate_satisfied_location THEN
      -- PENCEGAHAN CHAT NYASAR:
      -- Kunci partner secara asinkron menggunakan advisory lock.
      -- Jika gagal, berarti partner sedang memproses matching-nya sendiri, kita skip!
      IF pg_try_advisory_xact_lock(v_candidate.candidate_id) THEN
          ...
          EXIT;
      ELSE
          -- Gagal mendapatkan kunci, skip ke kandidat berikutnya
          CONTINUE;
      END IF;
    END IF;
  END LOOP;
