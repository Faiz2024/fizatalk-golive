...
BEGIN
  ...
  IF NOT v_am_shadowbanned AND v_shadowban_until IS NOT NULL THEN
    -- Bersihkan status kedaluwarsa sekali saja
  ...
  v_reputation := json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points, 'report_warnings_to_show', v_unacknowledged_reports_count, 'shadowbanned', v_am_shadowbanned, 'shadowban_until', v_shadowban_until);
  ...
  LOOP
    ...
    END IF;
    -- Shadowban hanya cocok dengan shadowban; normal hanya dengan normal
    v_candidate_shadowbanned := (v_candidate.candidate_shadowban_until IS NOT NULL AND v_candidate.candidate_shadowban_until > NOW());
    IF v_candidate_shadowbanned IS DISTINCT FROM v_am_shadowbanned THEN CONTINUE; END IF;
...
-- find_and_pair_partner: aturan pencocokan shadowban yang sama
...
BEGIN
  ...
  LOOP
    ...
    v_candidate_shadowbanned := (v_candidate.candidate_shadowban_until IS NOT NULL AND v_candidate.candidate_shadowban_until > NOW());
    IF v_candidate_shadowbanned IS DISTINCT FROM v_am_shadowbanned THEN
      CONTINUE;
    END IF;
