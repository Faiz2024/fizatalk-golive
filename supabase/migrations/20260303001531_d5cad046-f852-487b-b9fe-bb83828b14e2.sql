...
-- Update comprehensive_search_action to apply filters for ALL users (not just premium)
-- Non-premium users are limited by check_and_use_filter at UI level
...
BEGIN
  ...
  -- PERUBAHAN: Filter berlaku untuk SEMUA user yang punya target_gender/target_location yang di-set
  -- Non-premium dibatasi di level UI (10x/hari via check_and_use_filter)
  v_my_gender := v_user.gender;
  v_my_target_gender := v_user.target_gender;  -- Berlaku untuk semua user
  v_my_location := v_user.location;
  v_my_target_location := v_user.target_location;  -- Berlaku untuk semua user
  ...
  LOOP
    ...
    -- Filter kandidat juga berlaku untuk semua user
