...
DECLARE
  ...
  -- Filter eligibility vars
  v_filter_allowed BOOLEAN := FALSE;
  v_filter_uses_today INTEGER;
  v_filter_uses_date DATE;
  v_today_wib DATE;
  -- Candidate filter eligibility
  v_candidate_filter_allowed BOOLEAN;
  v_candidate_filter_uses_today INTEGER;
  v_candidate_filter_uses_date DATE;
BEGIN
  -- 1. UPSERT USER
  SELECT id, username, first_name, last_active, gender, state, partner_id,
         ...
         filter_uses_today, filter_uses_date
  INTO v_user
  FROM telegram_users
  WHERE id = p_user_id;
  ...
  -- Jika tombol NEXT ditekan, cek apakah USER INI (p_user_id) berhak dapat promo
  ...
  -- CEK APAKAH USER BERHAK MENGGUNAKAN FILTER
  -- Premium = selalu berhak. Non-premium = cek kuota harian.
  v_today_wib := (NOW() AT TIME ZONE 'Asia/Jakarta')::DATE;
  v_filter_uses_today := v_user.filter_uses_today;
  v_filter_uses_date := v_user.filter_uses_date;
  
  -- Reset jika hari sudah berganti (WIB)
  IF v_filter_uses_date < v_today_wib THEN
    v_filter_uses_today := 0;
  END IF;
  
  IF v_is_premium THEN
    v_filter_allowed := TRUE;
  ELSIF v_filter_uses_today < 10 THEN
    v_filter_allowed := TRUE;
  ELSE
    v_filter_allowed := FALSE;
  END IF;
  
  v_my_gender := v_user.gender;
  -- Hanya terapkan filter jika user berhak (premium atau masih ada kuota)
  IF v_filter_allowed THEN
    v_my_target_gender := v_user.target_gender;
    v_my_target_location := v_user.target_location;
  ELSE
    -- Kuota habis & bukan premium -> mode default (tanpa filter)
  ...
  FOR v_candidate IN
    SELECT 
      ...
      tu.filter_uses_today as candidate_filter_uses_today,
      tu.filter_uses_date as candidate_filter_uses_date
  ...
  LOOP
    ...
    -- Cek jika kandidat ada di history saya
    IF v_user.last_partners IS NOT NULL AND v_candidate.candidate_id = ANY(v_user.last_partners) THEN
        CONTINUE;
    END IF;
    -- Cek jika saya ada di history kandidat
    IF v_candidate.candidate_last_partners IS NOT NULL AND p_user_id = ANY(v_candidate.candidate_last_partners) THEN
        CONTINUE;
    END IF;
    
    -- CEK APAKAH KANDIDAT BERHAK MENGGUNAKAN FILTER
    v_candidate_is_premium := v_candidate.candidate_premium_until IS NOT NULL AND v_candidate.candidate_premium_until > NOW();
    v_candidate_filter_uses_today := v_candidate.candidate_filter_uses_today;
    v_candidate_filter_uses_date := v_candidate.candidate_filter_uses_date;
    
    IF v_candidate_filter_uses_date < v_today_wib THEN
      v_candidate_filter_uses_today := 0;
    END IF;
    
    IF v_candidate_is_premium THEN
      v_candidate_filter_allowed := TRUE;
    ELSIF v_candidate_filter_uses_today < 10 THEN
      v_candidate_filter_allowed := TRUE;
    ELSE
      v_candidate_filter_allowed := FALSE;
    END IF;
    
    -- Terapkan filter kandidat hanya jika berhak
    IF v_candidate_filter_allowed THEN
