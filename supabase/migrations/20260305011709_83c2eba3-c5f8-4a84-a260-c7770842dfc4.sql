...
-- Update comprehensive_search_action to check blocked_until for premium temp bans
...
BEGIN
  ...
  SELECT id, username, first_name, last_active, gender, state, partner_id,
         ...
         filter_uses_today, filter_uses_date, blocked_until
  INTO v_user
  FROM telegram_users
  WHERE id = p_user_id;
  ...
  -- 2b. CEK BLOKIR SEMENTARA (blocked_until untuk premium)
  ...
  -- Jika blocked_until sudah lewat, bersihkan
  IF v_blocked_until IS NOT NULL AND v_blocked_until <= NOW() THEN
  ...
  -- Premium users: NO warnings at 40/70, temp ban handled via blocked_until at 100
  ...
  -- Kurangi kesempatan filter setiap kali mencari partner (non-premium)
  IF NOT v_is_premium THEN
    UPDATE telegram_users 
    SET filter_uses_today = CASE 
          WHEN filter_uses_date < v_today_wib THEN 1
          ELSE filter_uses_today + 1 
        END,
