-- =============================================================================
-- Migration: Update Ban Duration to 15 Days & Create Reengagement View
-- =============================================================================

-- 1. Update comprehensive_search_action: 3 days → 15 days, reset spam_warnings on auto-unblock
...
BEGIN
  ...
  -- 1.5 AUTO-SYNC PENALTY POINTS TO BLOCKED_USERS
  ...
  -- 2. CEK BLOKIR (dengan auto-expire 15 hari)
  ...
  IF v_blocked_at IS NOT NULL THEN
    IF v_blocked_at + INTERVAL '15 days' > NOW() THEN
      -- Masih dalam masa blokir 15 hari
    ...
    ELSE
      -- Sudah lewat 15 hari, auto-unblock dan reset penalty + spam warnings
  ...
  -- 3. CEK CHANNEL JOIN
  -- TEMPORARILY DISABLED: To reduce churn rate for users > 7 days
  /*
...
-- 2. Create View for reengagement eligible users (excludes actively blocked users)
