-- 1. Drop blocked_until column from telegram_users since it is no longer used
ALTER TABLE public.telegram_users DROP COLUMN IF EXISTS blocked_until;

-- 2. Update submit_partner_report to prevent premium users from being blocked when penalty points >= 100
...
-- 3. Update comprehensive_search_action to remove blocked_until references and premium temp bans
...
-- 4. Drop filter_uses_today and filter_uses_date columns from telegram_users since free filtering limit is no longer used
ALTER TABLE public.telegram_users DROP COLUMN IF EXISTS filter_uses_today;
ALTER TABLE public.telegram_users DROP COLUMN IF EXISTS filter_uses_date;

-- 5. Drop check_and_use_filter function since it's no longer used
DROP FUNCTION IF EXISTS public.check_and_use_filter(bigint);
