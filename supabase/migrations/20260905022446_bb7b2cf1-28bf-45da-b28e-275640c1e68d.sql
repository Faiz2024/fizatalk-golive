ALTER TABLE public.telegram_users
  ADD COLUMN IF NOT EXISTS chats_since_premium_shadow integer NOT NULL DEFAULT 0;

CREATE OR REPLACE FUNCTION public.comprehensive_search_action(p_user_id bigint, p_username text DEFAULT NULL::text, p_first_name text DEFAULT NULL::text, p_is_next boolean DEFAULT false, p_target_partner_id bigint DEFAULT NULL::bigint)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_am_shadowbanned BOOLEAN := FALSE;
  v_shadowban_until TIMESTAMPTZ;
  v_candidate_shadowbanned BOOLEAN;
  v_candidate_premium_shadow BOOLEAN;
  v_am_premium_shadow BOOLEAN := FALSE;
  v_my_chats_since INT := 0;
  v_allowed BOOLEAN;
  v_partner_is_premium_shadow BOOLEAN := FALSE;
  v_reputation JSON;
  v_threshold INT := 10;
BEGIN
  ...
  SELECT id, username, first_name, last_active, gender, state, partner_id,
         ...
         shadowban_until, COALESCE(chats_since_premium_shadow, 0) as chats_since_premium_shadow
  ...
  -- Status shadowban (berlaku untuk semua user, termasuk premium)
  v_shadowban_until := v_user.shadowban_until;
  v_am_shadowbanned := (v_shadowban_until IS NOT NULL AND v_shadowban_until > NOW());
  IF NOT v_am_shadowbanned AND v_shadowban_until IS NOT NULL THEN
    UPDATE telegram_users SET shadowban_until = NULL, negative_reports_count = 0, last_negative_report_at = NULL, reports_decay_at = NULL
    WHERE id = p_user_id AND shadowban_until IS NOT NULL AND shadowban_until <= NOW();
    v_shadowban_until := NULL;
  ELSIF v_am_shadowbanned THEN
    PERFORM public.decay_negative_reports(p_user_id);
  END IF;
  ...
  v_am_premium_shadow := (v_am_shadowbanned AND v_is_premium);
  v_my_chats_since := COALESCE(v_user.chats_since_premium_shadow, 0);
  ...
  v_reputation := json_build_object('status', v_reputation_status, 'message', v_reputation_message, 'penalty_points', v_penalty_points, 'report_warnings_to_show', v_unacknowledged_reports_count, 'shadowbanned', v_am_shadowbanned, 'shadowban_until', v_shadowban_until, 'shadowban_notice', (v_am_shadowbanned AND NOT v_is_premium));

  IF v_reputation_status = 'banned' THEN
    RETURN json_build_object('success', false, 'error', 'user_banned', 'action', 'show_banned', 'reputation', v_reputation);
  END IF;
  ...
  IF p_is_next AND v_chat_ended THEN
    ...
    BEGIN
       ...
       IF v_my_promo->>'should_send' = 'true' THEN
          RETURN json_build_object('success', true, 'action', 'show_promo', 'chat_ended', true, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', v_reputation);
       END IF;
    END;
  END IF;
  ...
  IF NOT v_is_premium THEN
    ...
    IF v_should_send_channel_invite THEN
      RETURN json_build_object('success', true, 'action', 'show_channel_invite', 'should_send_channel_invite', true, 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', v_reputation);
    END IF;
  END IF;

  IF v_should_check_channel THEN
    RETURN json_build_object('success', true, 'action', 'needs_channel_check', 'should_check_channel', true, 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', v_reputation);
  END IF;

  IF public.check_and_claim_special_promo(p_user_id) THEN
    RETURN json_build_object('success', true, 'action', 'show_special_promo', 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'reputation', v_reputation);
  END IF;
  ...
  FOR v_candidate IN
    SELECT wq.user_id as candidate_id, tu.gender as candidate_gender, tu.target_gender as candidate_target_gender,
           ...
           tu.shadowban_until as candidate_shadowban_until,
           COALESCE(tu.chats_since_premium_shadow, 0) as candidate_chats_since
  ...
  LOOP
    ...
    v_candidate_shadowbanned := (v_candidate.candidate_shadowban_until IS NOT NULL AND v_candidate.candidate_shadowban_until > NOW());
    v_candidate_is_premium := v_candidate.candidate_premium_until IS NOT NULL AND v_candidate.candidate_premium_until > NOW();
    v_candidate_premium_shadow := (v_candidate_shadowbanned AND v_candidate_is_premium);

    -- Aturan pencocokan shadowban
    IF v_am_shadowbanned THEN
      IF v_candidate_shadowbanned THEN
        v_allowed := TRUE;
      ELSIF v_am_premium_shadow THEN
        -- Premium shadowban boleh bertemu user normal yang sudah >= 10 obrolan
        v_allowed := (v_candidate.candidate_chats_since >= v_threshold);
      ELSE
        v_allowed := FALSE;
      END IF;
    ELSE
      IF NOT v_candidate_shadowbanned THEN
        v_allowed := TRUE;
      ELSIF v_candidate_premium_shadow THEN
        v_allowed := (v_my_chats_since >= v_threshold);
      ELSE
        v_allowed := FALSE;
      END IF;
    END IF;
    ...
    IF v_i_satisfied_gender AND v_candidate_satisfied_gender AND v_i_satisfied_location AND v_candidate_satisfied_location THEN
      IF pg_try_advisory_xact_lock(v_candidate.candidate_id) THEN
          ...
          v_partner_is_premium_shadow := v_candidate_premium_shadow;
          EXIT;
      ELSE
          CONTINUE;
      END IF;
  ...
  IF v_partner_id IS NOT NULL THEN
    UPDATE telegram_users SET state = 'chatting', partner_id = v_partner_id,
      chats_since_premium_shadow = CASE
        WHEN v_am_shadowbanned THEN COALESCE(chats_since_premium_shadow, 0)
        WHEN v_partner_is_premium_shadow THEN 0
        ELSE COALESCE(chats_since_premium_shadow, 0) + 1 END,
    ...
    UPDATE telegram_users SET state = 'chatting', partner_id = p_user_id,
      chats_since_premium_shadow = CASE
        WHEN v_partner_is_premium_shadow THEN COALESCE(chats_since_premium_shadow, 0)
        WHEN v_am_premium_shadow THEN 0
        ELSE COALESCE(chats_since_premium_shadow, 0) + 1 END,
  ...
  ELSE
    ...
    RETURN json_build_object('success', true, 'matched', false, 'partner_id', NULL, 'action', 'wait_in_queue', 'chat_ended', v_chat_ended, 'old_partner_id', v_old_partner_id, 'old_partner_promo', v_old_partner_promo, 'should_check_channel', v_should_check_channel, 'reputation', v_reputation);
  END IF;
END;
$function$;
...
BEGIN
  SELECT
    ...
    premium_until, state, partner_id, last_partners, shadowban_until,
    COALESCE(chats_since_premium_shadow, 0) AS chats_since_premium_shadow
  INTO v_user
  FROM telegram_users
  WHERE id = p_user_id
  FOR UPDATE;
  ...
  v_am_shadowbanned := (v_user.shadowban_until IS NOT NULL AND v_user.shadowban_until > NOW());
  ...
  IF v_partner_id IS NOT NULL THEN
    ...
    RETURN json_build_object('success', true, 'status', 'matched', 'partner_id', v_partner_id);
  ELSE
    ...
    RETURN json_build_object('success', true, 'status', 'waiting', 'partner_id', NULL);
  END IF;
END;
$function$;