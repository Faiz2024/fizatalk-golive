...
--- ROUTINE: comprehensive_search_action ---
...
BEGIN
  ...
  -- Uncomment the IF block below to reactivate channel join requirement
  /*
...
--- ROUTINE: search_or_next_partner ---
...
BEGIN
  ...
  -- Jika user banned, return langsung
  IF v_reputation_status = 'banned' THEN
    RETURN json_build_object(
      'success', false,
      'error', 'user_banned',
  ...
  -- =========================================
  -- 3. HANDLE NEXT: AKHIRI CHAT JIKA SEDANG CHATTING
  -- =========================================
  IF p_is_next AND v_user.state = 'chatting' AND v_user.partner_id IS NOT NULL THEN
    v_old_partner_id := v_user.partner_id;
  ...
  -- Jika user sedang dalam state waiting, cek apakah benar ada di queue
  IF v_user.state = 'waiting' AND NOT p_is_next THEN
    -- Cek apakah user benar ada di queue
    IF EXISTS (SELECT 1 FROM waiting_queue WHERE user_id = p_user_id) THEN
      RETURN json_build_object(
        'success', false,
